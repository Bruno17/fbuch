...
if (!isset($scriptProperties['download']) || !($scriptProperties['download'])) {
    ...
    if ($date_object = $modx->getObject('fbuchDate', array('id' => $object_id))) {
        $type_object = $date_object->getOne('Type');
        //print_r($type_object->toArray());
        $exportrow_hooksnippet = $type_object->get('exportrow_hooksnippet');
    }
    ...
    if ($collection = $modx->migx->getCollection($c)) {
        ...
        foreach ($collection as $tempRow) {
            ...
            $tempRow['createdon'] = strftime('%d.%m.%Y %H:%M',strtotime($tempRow['createdon']));
            if (!empty($exportrow_hooksnippet)) {
                $snippetProperties = 
                ['row' => &$tempRow,
                 'exportFields' => &$exportFields
                ];
                $modx->runSnippet($exportrow_hooksnippet, $snippetProperties);
            }
    ...
    $cacheName = $classname . '.csv';
    $cacheName = $modx->getOption('core_path') . 'components/fbuch/export/' . $cacheName;
